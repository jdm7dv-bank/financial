...
//** starting 2004
//** 14 Jun09 comments
//** 14 Sep 09 More commments, web references and exercise
//** 17 Sep 09 30_360 30_360E 30_360E_ISDA
//**  5 Dec 09 IMMDate(int Month, int Year), IMMDate(), IMM_Date_Nth(Date Today, int NthStir), IMM_Date_Nth(int NthStir) 
...
// Exercise:
// 1) add method to complete all convenction you find to following address:
// http://en.wikipedia.org/wiki/Day_count_convention
...
public class Date
{
    ...
    //Contructor for i.e. for 15 Sep 10 use Date(2010,9,15)
	public Date (int Year, int Month, int Day)
	{
		my_date = new DateTime(Year,Month,Day);	
	}

    //Contructor using Class DateTime
	public Date (DateTime MyDateTime)
	{
		my_date = MyDateTime;
	}

    //Contructor using Excel serial number i.e for 15 Sep 10  use Date(40436)
    ...
    //Please see "Pricing and hedging swaps" by Paul Miron pag 12
    ...
    //Method Member: Calculate YearFraction switching between the previous method, this is just an utility
    public double YF_Number(int number, Date Date2)
    {
        double outPut = 99;
        switch (number)
        {
            ...
            case 1:
                outPut = this.YF_30_360(Date2);
                break;
            case 2:
                outPut = this.YF_MM(Date2);
                break;
            case 3:
                outPut = this.YF_365(Date2);
                break;
            case 4:
                outPut = this.YF_AA(Date2);
                break;
            case 5:
                outPut = this.YF_365_25(Date2);
                break;
            case 6://9jul06
                outPut = this.YF_AAL(Date2);
                break;
    ...
    //Method Member: calculte numbers of days between two dates according YF_MM
	public double D_MM(Date Date2)
	{
		return YF_MM(Date2)*360;
	}

    //Method Member: calculte numbers of days between two dates according YF_365
	public double D_365(Date Date2)
	{
		return YF_365(Date2)*365;
	}

    //Method Member: calculte numbers of days between two dates according YF_BB
	public double D_BB(Date Date2)
	{
		return YF_BB(Date2)*360;
	}

     //Method Member: calculte numbers of days between two dates according YF_30_360
	public double D_30_360(Date Date2)
	{
		return YF_30_360(Date2)*360;
	}

    //Method Member: calculte numbers of days between two dates according YF_30_360E_ISDA
	...
	public double D_30_360E_ISDA(Date Date2,Date TerminationDate)
	{
     ...
     //Method Member: calculte numbers of days between two dates according YF_BBE
	public double D_30_360E(Date Date2)
	{
		return YF_30_360E(Date2)*360;
	}

    //Method Member: calculte numbers of days between two dates 
    ...
    // in which case tha date is rolled on the previous business day. (see http://en.wikipedia.org/wiki/Accrued_interest
    ...
    public static double[] GetFullCouponArray(Date[] FromCpnDt, Date[] ToCpnDt, double CpnRate, double FaceValue, Dc dayCount)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            //To get the full coupon the settlement date sould be equal to nextCpnDate of AccruedInterestMethod
    ...
    //Method Member: add "months", where months is an int, modified following actived
    ...
    public static Date IMMDate(int Month, int Year)
    {
        ...
        DateTime outPut = new DateTime(Year, Month, 1).AddDays(-1); //I start with last day of prev. month
    ...
    //Method Member: It calculates the Nth nearest IMM Date of a quaterly futures (Mar,Jun,Sep,Dec), given Today and NthStir
    ...
    //Binary opetator: subtract n_days 
	public static Date operator - (Date my_date2,int n_days)
	{
		DateTime OutDate = new DateTime( );
		OutDate = my_date2.DateValue.AddDays(-n_days);
