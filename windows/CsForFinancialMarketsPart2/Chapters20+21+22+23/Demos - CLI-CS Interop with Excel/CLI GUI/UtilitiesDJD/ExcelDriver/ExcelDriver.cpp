...
void ExcelDriver::AddAssocMatrix(const std::string& name, 
		NumericMatrix<double, long>* matrix, 
		const Set<double>&  rowLabels, const Set<double>&  columnLabels)
	{
		
	// Convert sets to list
		Set<double>::const_iterator iter = rowLabels.begin();
		list<string> rows;
		list<string> columns;

		for (iter = rowLabels.begin(); iter != rowLabels.end(); ++iter)
		{
			rows.push_back(getString(*iter));
		}

		for (iter = columnLabels.begin(); iter != columnLabels.end(); ++iter)
		{
			columns.push_back(getString(*iter));
		}

		AddMatrix(name, *matrix, rows, columns);
}
