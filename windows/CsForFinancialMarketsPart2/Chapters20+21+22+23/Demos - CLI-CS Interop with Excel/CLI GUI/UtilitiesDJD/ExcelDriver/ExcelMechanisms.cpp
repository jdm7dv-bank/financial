...
void printAssocMatrixInExcel(AssocMatrix<double, double, double>& matrix, const std::string& SheetName)
{
	ExcelDriver& excel = ExcelDriver::Instance();
	excel.MakeVisible(true);
	
	Set<double> dd = matrix.ColumnKeys();
	Set<double> cc = matrix.RowKeys();
	
	excel.AddAssocMatrix(SheetName, (matrix.mat), cc, dd);
...
void printDifferenceInExcel(const Vector<double, long>& x,	const Vector<double, long>& y1,					
					...
					const std::string& horizontal, 
					const std::string& vertical,
					const std::string& legend, long frequency)// frequencey not used
{
...
void printDiscreteFunctionValues(double (*f) (double x, double y), double A, double B,long nSteps, 
...
// Create discrete x and y arrays
	...
	Vector<double, long> mesh2(ySteps + 1, 1);
	double val2 = C;
	for (long i = 1; i <= ySteps + 1; i++)	
	{
		mesh2[i] = val2;
		val2 += h;
	}
	...
	// Now array of values
	NumericMatrix<double, long> result(nSteps + 1, ySteps + 1, 1, 1);

	for (long i2 = 1; i2 <= nSteps + 1; i2++)	
	{
		for (long j2 = 1; j2 <= ySteps + 1; j2++)
		{
			result(i2, j2) = (*f)(mesh[i2], mesh2[j2]);
		}
	}
	...
	printMatrixInExcel(result, mesh, mesh2, string("2d curve"));
}
